...
class DownloadTracker:
    ...
    def is_downloaded_but_not_processed(self, dataset_id: str) -> bool:
        """Check if dataset is downloaded but not yet processed"""
        if dataset_id not in self.tracked_datasets:
            return False
        ...
        # Check if processing is NOT complete
        if dataset_info.get('processing_complete', False):
            return False
        
        # Check if raw files still exist
        dataset_path = Path(dataset_info.get('dataset_path', ''))
        if not dataset_path.exists():
            logger.info(f"Dataset {dataset_id} marked as downloaded but files missing, re-downloading")
            return False
...
class StreamingPipeline:
    ...
    def load_config(self, config_path: str) -> Dict:
        ...
        log_file = log_config.get('log_file', 'streaming_pipeline.log')
    ...
    # Removed get_next_batch method - using individual dataset processing instead
    ...
    def download_dataset(self, dataset_info: Dict) -> Optional[str]:
        ...
        # Check if downloaded but not processed
        if self.download_tracker.is_downloaded_but_not_processed(dataset_id):
            logger.info(f"Dataset {dataset_id} already downloaded but not processed, using existing files")
            dataset_info = self.download_tracker.tracked_datasets[dataset_id]
            return dataset_info.get('dataset_path', str(self.dirs['raw'] / dataset_id))
    ...
    def try_download_individual_files(self, dataset_id: str, output_dir: Path, data_type: str) -> bool:
        ...
        try:
            ...
            if 'MTX' in data_type.upper():
                file_types.extend(['*.mtx', 'matrix.mtx*', '*.mtx.gz', 'matrix.mtx.gz'])
            if 'TSV' in data_type.upper():
                file_types.extend(['*.tsv', '*.txt', '*.tsv.gz', '*.txt.gz'])
            if 'CSV' in data_type.upper():
                file_types.extend(['*.csv', '*.csv.gz'])
            if 'TXT' in data_type.upper():
                file_types.extend(['*.txt', '*.txt.gz'])
        ...
        if self.download_tracker.is_downloaded(dataset_id):
            ...
            if processed_path.exists():
                try:
                    ...
                    # Still need to clean up raw data even for cached datasets
                    storage_config = self.config.get('storage_management', {})
                    keep_raw_data = storage_config.get('keep_raw_data', False)
                    
                    if not keep_raw_data:
                        # Find the raw data directory for this dataset
                        raw_dir = self.dirs['raw'] / dataset_id
                        if raw_dir.exists():
                            try:
                                shutil.rmtree(raw_dir)
                                logger.info(f"✅ Successfully cleaned up raw data for cached dataset {dataset_id}")
                            except Exception as e:
                                logger.error(f"❌ Failed to clean up raw data for cached dataset {dataset_id}: {e}")
                        else:
                            logger.info(f"ℹ️ No raw data to clean up for cached dataset {dataset_id} (already cleaned or never existed)")
                    else:
                        logger.info(f"📁 Keeping raw data for cached dataset {dataset_id} (keep_raw_data=True)")
    ...
    def extract_nested_archives(self, dataset_dir: Path):
        ...
        for tar_gz_file in tar_gz_files:
            try:
                ...
                logger.info(f"Successfully extracted {tar_gz_file.name} to {extract_dir}")
    ...
    def find_10x_directories(self, dataset_dir: Path) -> List[Path]:
        """Find directories that contain 10X Genomics matrix files"""
        tenx_dirs = []
        
        # Look for directories containing 10X matrix files
        for root, dirs, files in os.walk(dataset_dir):
            root_path = Path(root)
            
            # Check if this directory contains 10X matrix files
            ...
            # If it has matrix and at least one other 10X file, it's a 10X directory
    ...
    def find_data_files(self, dataset_path: str, data_type: str) -> List[str]:
        """Find data files in the dataset directory with improved pattern matching and nested archive support"""
        dataset_dir = Path(dataset_path)
        data_files = []
        
        # First, try to find and extract nested archives
        self.extract_nested_archives(dataset_dir)
        ...
        file_patterns = {
            ...
            'H5AD': ['*.h5ad', '*.h5'],
            'MTX': ['matrix.mtx*', '*.mtx', '*matrix.mtx*', '*.mtx.gz', 'matrix.mtx.gz'],
            'TSV': ['*.tsv', '*counts.tsv', '*expression.tsv', '*.tsv.gz', '*counts.tsv.gz', '*expression.tsv.gz'],
            'RDS': ['*.rds', '*seurat.rds', '*sce.rds'],
            'LOOM': ['*.loom', '*expression.loom'],
            'CSV': ['*.csv', '*counts.csv', '*expression.csv', '*.csv.gz', '*counts.csv.gz', '*expression.csv.gz'],
            'TXT': ['*.txt', '*counts.txt', '*expression.txt', '*.txt.gz', '*counts.txt.gz', '*expression.txt.gz'],
        ...
        # Special handling for 10X Genomics data in nested structure
        # Look for directories that contain 10X matrix files
        tenx_dirs = self.find_10x_directories(dataset_dir)
        for tenx_dir in tenx_dirs:
            # Add the directory path as a potential data file
            data_files.append(str(tenx_dir))
    ...
    def load_data_file(self, file_path: str, dataset_info: Dict) -> Optional[ad.AnnData]:
        ...
        try:
            if file_path.suffix == '.h5ad':
                ...
                # Handle 10X directories (from nested archives)
                return self.read_10x_mtx(file_path)
            else:
    ...
    def read_compressed_file(self, file_path: Path, dataset_info: Dict) -> Optional[ad.AnnData]:
        ...
        try:
            ...
            if 'matrix.mtx' in file_path.name:
                ...
                if file_path.suffix == '.gz' and any(ext in file_path.name.lower() for ext in ['.csv', '.tsv', '.txt']):
                    logger.info(f"Reading compressed CSV/TXT file: {file_path.name}")
                    ...
                    if '.csv' in file_path.name.lower():
                        ...
                        sep = '\t'  # Default to tab for .txt files
                    
                    # Read compressed file
                    with gzip.open(file_path, 'rt') as f:
                        # Try to read as matrix (genes as columns, cells as rows)
                        try:
                            df = pd.read_csv(f, sep=sep, index_col=0)
                            
                            # Check if this looks like a count matrix
                            if df.shape[0] > 10 and df.shape[1] > 10:  # Reasonable matrix size
                                # Transpose to get cells as rows, genes as columns
                                adata = ad.AnnData(
                                    X=df.T.values,
                                ...
                                logger.info(f"Successfully loaded compressed matrix file: {adata.n_obs} cells, {adata.n_vars} genes")
                                return adata
                            else:
                                logger.warning(f"File {file_path.name} doesn't appear to be a count matrix")
                                return None
                                
                        except Exception as e:
                            logger.warning(f"Failed to read {file_path.name} as matrix: {e}")
                            return None
                else:
    ...
    def quality_control(self, adata: ad.AnnData, dataset_info: Dict) -> ad.AnnData:
        ...
        try:
            ...
            max_mt = qc_config.get('max_mt_percent', 20)
            ...
            # Filter genes using config parameter
    ...
    def normalize_data(self, adata: ad.AnnData, dataset_info: Dict) -> ad.AnnData:
        ...
        # Normalize using config parameter
    ...
    def save_processed_dataset_individual(self, adata: ad.AnnData, dataset_info: Dict):
        """Save individual processed dataset without batching"""
        dataset_id = dataset_info['dataset_id']
        ...
        stats_path = self.dirs['outputs'] / f"{dataset_id}_stats.json"
        stats = {
            'dataset_id': dataset_id,
            'n_cells': adata.n_obs,
            ...
            'processing_timestamp': datetime.now().isoformat(),
            'data_type': dataset_info.get('data_type', ''),
            'repository': dataset_info.get('repository', '')
        }
        
        with open(stats_path, 'w') as f:
            json.dump(stats, f, indent=2)
        
        logger.info(f"Saved individual processed dataset: {dataset_id} ({adata.n_obs} cells, {adata.n_vars} genes)")
    
    def run_pipeline(self):
        """Run the memory-optimized streaming pipeline - ONE DATASET AT A TIME"""
        self.print_status("Starting pipeline", "Initializing MEMORY-OPTIMIZED streaming processing")
        logger.info("Starting streaming pipeline - MEMORY OPTIMIZED MODE")
        
        # Load all datasets
        datasets_df = self.load_dataset_list()
        if datasets_df.empty:
            logger.error("No datasets found in the list. Exiting.")
            return
        ...
        self.print_status("Pipeline initialized", f"Total: {total_datasets} datasets (STREAMING MODE - No Batching)")
        
        # Process datasets ONE BY ONE (no batching to save memory)
        for i, dataset_info in enumerate(datasets):
            dataset_id = dataset_info['dataset_id']
            self.status_info['overall_progress'] = (i, total_datasets)
            
            # Check memory before processing
            memory_info = psutil.virtual_memory()
            self.print_status("Memory Check", f"Available: {memory_info.available / (1024**3):.1f}GB, Used: {memory_info.percent:.1f}%")
            ...
            # Process dataset
            adata = self.process_dataset(dataset_path, dataset_info)
            if adata is None:
                continue
            
            # Save processed data immediately (no batching)
            self.save_processed_dataset_individual(adata, dataset_info)
            ...
            if not keep_raw_data:
                ...
                if raw_dir.exists():
                    try:
                        shutil.rmtree(raw_dir)
                        logger.info(f"✅ Successfully cleaned up raw data for {dataset_id}")
                    except Exception as e:
                        logger.error(f"❌ Failed to clean up raw data for {dataset_id}: {e}")
                else:
                    logger.info(f"ℹ️ No raw data directory found for {dataset_id} (already cleaned or never existed)")
            else:
                logger.info(f"📁 Keeping raw data for {dataset_id} (keep_raw_data=True)")
            ...
            memory_info = psutil.virtual_memory()
            self.print_status("Memory Cleanup", f"Available: {memory_info.available / (1024**3):.1f}GB, Used: {memory_info.percent:.1f}%")
            
            self.print_status("Dataset complete", f"Processed {i+1}/{total_datasets} datasets")
            logger.info(f"Completed dataset {i+1}/{total_datasets}: {dataset_id}")
